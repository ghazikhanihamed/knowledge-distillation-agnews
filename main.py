...
print("\nLoading models...")
teacher_model_name = "./bert_teacher_ag_news_improved"
...
student_model = AutoModelForSequenceClassification.from_pretrained(
    student_model_name, num_labels=4, dropout=0.2
).to(device)
...
class DistillationTrainer(Trainer):
    ...
    def compute_loss(
        ...
        loss_ce = nn.CrossEntropyLoss(label_smoothing=0.1)(
            student_logits.view(-1, 4), labels.view(-1)
        )
        with torch.no_grad():
...
with open(results_file, "w") as f:
    ...
    f.write("=" * 70 + "\n")
    f.write(
        f"Settings: lr_enc=3e-5, lr_dec=2e-5, temp=5.0 (DistilBERT), 2.0 (DistilGPT2), alpha=0.5 (DistilBERT), 0.5 (DistilGPT2), epochs=3, batch_size=32\n"
    )
    f.write(f"{len(train_data)} training examples, {len(test_data)} test examples\n")
...
acc_improvement = distill_results["eval_accuracy"] - baseline_results["eval_accuracy"]
...
times = ["N/A", f"{baseline_time:.1f}s", f"{distill_time:.1f}s", f"{decoder_time:.1f}s"]
